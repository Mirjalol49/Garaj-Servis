    ...
    <div className="max-w-6xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">Owner overview</h1>
        </div>
        <Link href="/finance" className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm text-foreground hover:border-primary/50 hover:text-primary">
          Finance
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {[
          { label: 'Income', value: summary.income, icon: CircleDollarSign, color: 'text-primary' },
          { label: 'Expense', value: summary.expense, icon: ReceiptText, color: 'text-[#ffba79]' },
          { label: 'Profit', value: summary.profit, icon: TrendingUp, color: summary.profit >= 0 ? 'text-primary' : 'text-destructive' },
          { label: 'Net Cashflow', value: summary.netCashflow, icon: Banknote, color: summary.netCashflow >= 0 ? 'text-primary' : 'text-destructive' },
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-border bg-card">
          <div className="border-b border-border px-5 py-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground font-[var(--font-geist)]">Recent jobs</p>
          </div>
          {recentJobs.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {recentJobs.map((job) => (
                <Link key={job.id} href={`/jobs/${job.id}`} className="grid grid-cols-[1fr_auto] gap-3 px-5 py-4 hover:bg-muted">
                  <div>
                    <p className="font-mono text-sm font-semibold text-foreground">{job.job_number}</p>
                    <p className="text-xs text-muted-foreground">{[job.plate_number, job.company_name].filter(Boolean).join(' · ')}</p>
                  </div>
                  <p className="text-xs capitalize text-muted-foreground">{statusLabel(job.status)}</p>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="rounded-2xl border border-border bg-card">
          <div className="border-b border-border px-5 py-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground font-[var(--font-geist)]">Top receivables</p>
          </div>
          {receivables.length === 0 ? (
            <p className="px-5 py-6 text-sm text-muted-foreground">No receivables yet.</p>
          ) : (
            <div className="divide-y divide-border">
