  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-foreground">Ledger, Cashflow & Reports</h1>
          </div>
          <div className="flex flex-wrap justify-end gap-2">
            {[
              ...
              <a
                ...
                className="rounded-xl border border-border px-3 py-2 text-xs font-semibold text-foreground hover:border-primary/50 hover:text-primary"
              >
                {label}
              </a>
            ))}
      ...
      <div className="grid gap-3 md:grid-cols-6">
        {[
          { label: 'Income', value: summary.income, color: 'text-primary' },
          { label: 'Expense', value: summary.expense, color: 'text-[#ffba79]' },
          { label: 'Profit', value: summary.profit, color: summary.profit >= 0 ? 'text-primary' : 'text-destructive' },
          { label: 'Cashflow', value: summary.netCashflow, color: summary.netCashflow >= 0 ? 'text-primary' : 'text-destructive' },
          ...
          <div key={metric.label} className="rounded-2xl border border-border bg-card px-4 py-3">
            <p className="text-xs uppercase tracking-widest text-muted-foreground font-[var(--font-geist)]">{metric.label}</p>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card">
          ...
          {jobProfitability.length === 0 ? (
            <p className="px-5 py-6 text-sm text-muted-foreground">No jobs to report.</p>
          ) : (
            <div className="divide-y divide-border">
              {jobProfitability.slice(0, 8).map((job) => (
                <div key={job.jobId} className="grid grid-cols-[1fr_auto] gap-3 px-5 py-4">
                  <div>
                    <p className="font-mono text-sm font-semibold text-foreground">{job.jobNumber}</p>
                    <p className="text-xs text-muted-foreground">{[job.plateNumber, job.companyName].filter(Boolean).join(' · ')}</p>
                  </div>
                  <p className={`font-mono text-sm font-semibold ${job.grossProfit >= 0 ? 'text-primary' : 'text-destructive'}`}>
        ...
        <div className="rounded-2xl border border-border bg-card">
          ...
          {receivables.length === 0 ? (
            <p className="px-5 py-6 text-sm text-muted-foreground">No company balances.</p>
          ) : (
            <div className="divide-y divide-border">
              {receivables.slice(0, 8).map((row) => (
                <div key={row.companyId} className="grid grid-cols-[1fr_auto] gap-3 px-5 py-4">
                  <div>
                    <p className="text-sm font-medium text-foreground">{row.companyName}</p>
                    <p className="text-xs text-muted-foreground">Paid {formatMoney(row.paid)} of {formatMoney(row.revenue)}</p>
                  </div>
        ...
        <div className="rounded-2xl border border-border bg-card">
          ...
          {payables.length === 0 ? (
            <p className="px-5 py-6 text-sm text-muted-foreground">No master balances.</p>
          ) : (
            <div className="divide-y divide-border">
              {payables.slice(0, 8).map((row) => (
                <div key={row.masterId} className="grid grid-cols-[1fr_auto] gap-3 px-5 py-4">
                  <div>
                    <p className="text-sm font-medium text-foreground">{row.masterName}</p>
                    <p className="text-xs text-muted-foreground">Paid {formatMoney(row.paid)} of {formatMoney(row.earned)}</p>
                  </div>
      ...
      <div className="rounded-2xl border border-border bg-card">
        ...
        {entries.length === 0 ? (
          <p className="px-5 py-6 text-sm text-muted-foreground">No ledger entries recorded.</p>
        ) : (
          <div className="divide-y divide-border">
            {entries.slice(0, 100).map((entry) => (
              <div key={entry.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1.2fr_1fr_1fr_auto]">
                <div>
                  <p className="text-sm font-medium capitalize text-foreground">{entryLabel(entry.entry_type)}</p>
                  {entry.description && <p className="mt-1 text-xs text-muted-foreground">{entry.description}</p>}
                </div>
                <p className="text-sm text-muted-foreground">{new Date(entry.entry_date).toLocaleDateString('en-GB')}</p>
                <p className="text-sm capitalize text-muted-foreground">{entry.payment_method?.replaceAll('_', ' ') ?? 'No method'}</p>
